...
{
    FisherInformationManager::FisherInformationManager(std::shared_ptr<nav2_util::LifecycleNode> node)
    {
        node_ = node;
...
////////////////////////////////////////////////////////////////
    ...
    bool FisherInformationManager::isPoseSafe(geometry_msgs::msg::Pose& given_pose, bool exhaustiveSearch, float& information)
    {
        ...
        for (const auto& map_point : response->map_points)
        {
            ...
            if (std::isnan(info))
            {
                LOG_INFO("Information nan for: " << point_in_camera.x() << " y: " << point_in_camera.y() << " z: " << point_in_camera.z());
                continue;
            }
    ...
    float FisherInformationManager::getInformationFromLookup(Eigen::Vector3f& landmark_camera_frame, float step_max, float step_min, unsigned int latest_version)
    {
        ...
        if (it != lookup_table_fi_.end()) { 
            ...
            LOG_INFO("Key NOT found in the lookup table >> " + std::to_string(search_key.components[0]) + ", " + std::to_string(search_key.components[1]) + ", " + std::to_string(search_key.components[2]));
            return std::numeric_limits<float>::quiet_NaN();
