...
namespace frontier_exploration
{
    ...
    std::vector<Frontier> FrontierSearchAllCells::searchAllCells()
    {
        ...
        for (unsigned int mx = 0; mx < size_x_; ++mx)
        {
            for (unsigned int my = 0; my < size_y_; ++my)
            {
                ...
                if (isNewFrontierCell(idx, frontier_flag))
                {
                    ...
                    std::vector<Frontier> new_frontiers = buildNewFrontier(idx, frontier_flag);
    ...
    std::vector<Frontier> FrontierSearchAllCells::buildNewFrontier(unsigned int initial_cell, std::vector<bool> &frontier_flag)
    {
