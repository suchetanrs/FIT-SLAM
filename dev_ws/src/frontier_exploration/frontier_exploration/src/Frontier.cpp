...
size_t Frontier::getUID() const {
    if(unique_id == nullptr) throw std::runtime_error("A frontier property is null");
    return *unique_id;
}

int Frontier::getSize() const {
    if(size == nullptr) throw std::runtime_error("A frontier property is null");
    return *size;
}

geometry_msgs::msg::Point Frontier::getGoalPoint() const {
    if(goal_point == nullptr) throw std::runtime_error("A frontier property is null");
    return *goal_point;
}

geometry_msgs::msg::Quaternion Frontier::getGoalOrientation() const {
    if(best_orientation == nullptr || theta_s_star == nullptr) throw std::runtime_error("A frontier property is null");
    return *best_orientation;
}

double Frontier::getArrivalInformation() const {
    if(information == nullptr) throw std::runtime_error("Arrival information frontier property is null");
    return *information;
}

double Frontier::getPathLength() const {
    if(path_length == nullptr) throw std::runtime_error("Path length frontier property is null");
    return *path_length;
}

double Frontier::getFisherInformation() const {
    if(fisher_information_in_path == nullptr) throw std::runtime_error("A frontier property is null");
...
double Frontier::getWeightedCost() const {
    if(weighted_cost == nullptr) throw std::runtime_error("Weighted cost frontier property is null");
