...
namespace frontier_exploration
{
     ...
     */
    double polygonPerimeter(const geometry_msgs::msg::Polygon &polygon)
    {
     ...
     */
    template <typename T>
    bool pointInPolygon(const T &point, const geometry_msgs::msg::Polygon &polygon)
    {
        int cross = 0;
        for (int i = 0, j = polygon.points.size() - 1; i < polygon.points.size(); j = i++)
        {
    ...
    /**
     * @brief Outputs a geometry_msgs::Quaternion from a yaw angle. Sets roll and pitch to 0.
